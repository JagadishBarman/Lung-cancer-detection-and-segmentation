function [ Ann2 ] = intr_volume( slic_thick,spacing,d, vol )
% 內差法+曲線擬和>>誤差:0.35%
...
xlabel('Slice No.'), ylabel('diameter')%,'FontWeight','bold'
% xx=(1/m)*l_uh;
% xx2=n+(1/m)*l_dh;
xx=l_uh;
xx2=n+l_dh;
...
%    A_down=0;
% end
% Ann3=(An+A_up*(l_uh/m)+A_down*(l_dh/m));
Ann3=((sum(Axx)*(1/m))+A_up*(l_uh/m)+A_u*(l_uh/m)+(sum(Acx2)*(l_dh/m))+A_d*(l_dh/m));
% Ann3=((sum(Axx)*(1/m))+A(end-1)+A_u*(l_uh)+A_down*(l_dh/m));
